...
与上表相关的 **仓库 `cloud/` 实现注记**（字段级仍以 OpenAPI 为准）：`POST /v1/jobs` 与 `POST .../print-ack` 的 **`Idempotency-Key`** 按 **`device_id` + 键** 组合去重；`POST .../chunks` 提供 **`seq` / `final` 分片序号桩**（不落盘音频字节）；**`POST /v1/devices/telemetry`** 为 **§2.4.3 设备→云遥测** 的 **HTTPS 桩**（与 MQTT 主题并列/兜底，**禁止**儿音原文类字段）；**`MQTT_URL` + `MQTT_SUBSCRIBE_TELEMETRY=1`** 时 **`device-api`** 订阅 **`devices/+/telemetry`** 作云端摄入桩（指标见 `fancy_print_device_telemetry_mqtt_received_total`）；网关与各 API 透传 **`traceparent` / `tracestate`**（W3C Trace Context）与 **`X-Request-Id`**，见 [`contracts/openapi/device-v1-mvp.yaml`](../contracts/openapi/device-v1-mvp.yaml)；**mTLS** 终结见 [`cloud/docs/ingress-mtls.md`](../cloud/docs/ingress-mtls.md)。
...
上表 **MVP** 行描述的是 **量产云端应交付的能力边界**（含真实 ASR / 文生图 / 图审 / 对象存储与家长 OIDC 等）。仓库内 **`cloud/` 当前迭代** 以 **协议与编排骨架** 为主：路由、设备 JWT、Job 状态与轮询、策略 ETag、artifact 跳转形态、MQTT 发布与（可选）**遥测主题订阅**、HTTPS 遥测入口、网关与家长 BFF 分面等已与 **§2.4** 对齐。**`VendorStubsService`** 仍为 **ASR 文案 / 预览** 的默认占位；**`VendorFacadeService`** 在配置 **`ASR_HTTP_URL`**、**`IMAGE_GEN_HTTP_URL`**、**`S3_PREVIEW_*`** 时走 **HTTP 供应商** 与 **S3 预签名 URL**（GCS 等可由生图服务返回 **`image_url`**）。**证书序列号 → `device_id`** 可在本仓库 **`gateway` 进程内 HTTPS+mTLS**（**`GATEWAY_MTLS_SERIAL_MAP_JSON`**）完成，经 **`x-device-id-from-mtls`** 与 **`POST /v1/auth/mtls`** 换票；Ingress 终结 mTLS 的路径仍见 **`cloud/docs/ingress-mtls.md`**。**`edge/cloud-connector`** 提供 **真实 MQTT 客户端** 遥测发布示例。各厂商原生 SDK、完整 PKI 注册表与 **§5.2** 全量对象存储策略等，仍须在选定供应商后补强。
