...
上表 **MVP** 行描述的是 **量产云端应交付的能力边界**（含真实 ASR / 文生图 / 图审 / 对象存储与家长 OIDC 等）。仓库内 **`cloud/` 当前迭代** 以 **协议与编排骨架** 为主：路由、设备 JWT、Job 状态与轮询、策略 ETag、artifact 跳转形态、MQTT 发布钩子、网关与家长 BFF 分面等已与 **§2.4** 对齐；**不替代** 各 Adapter 的厂商 SDK、密钥注入与 **§5.2** 签名对象存储等 **§8 落地工作**，后者在选定供应商与 staging 密钥后按 Adapter 接入即可替换桩实现。
