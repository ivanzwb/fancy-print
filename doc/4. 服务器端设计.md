...
> **定位**：描述 **设备经 `cloud-connector` 访问的云端能力** 的逻辑架构、服务边界、协议与安全原则；并给出 **云端功能列表** 与 **HTTP/MQTT API 一览**（**§2.3～§2.4**），与仓库 [`contracts/openapi/`](../contracts/openapi/) 及 MQTT 约定对齐演进。**仓库 `cloud/` 细化部署与模块映射**见 **§2.2.1**；**ASR / 文生图 / 审核** 的环境变量、驱动与扩展方式见 **§2.2.2**（与 `device-api` 中 `adapters/vendors/`、`vendor-adapter.factory.ts` 对齐）。**不替代** [`0. 产品构想.md`](0. 产品构想.md) / [`1. 项目计划书.md`](1. 项目计划书.md) 中的商业与 PRD 细节。端侧软件分层、进程与主流程见 [`3. 端侧设计.md`](3. 端侧设计.md)；OS 细节、清单、外设与样机硬件见 [`2. 端侧软件与工程样机技术分析.md`](2. 端侧软件与工程样机技术分析.md) **§1～§13**；**端云总览图**见下节嵌入图（源文件 [`images/系统架构图.svg`](images/系统架构图.svg)）。
...
flowchart TB
  ...
  subgraph dapi["device-api apps device-api"]
    ...
    JM["JobsModule：Job 状态机、幂等、分片、POST advance + Redis 推进锁、可选 BullMQ"]
  ...
  subgraph pbff["parent-bff apps parent-bff"]
    ...
    PMQ["MqttModule：发布 approval、policy_push"]
    PH --> PMQ
  end
  ...
  GW -->|"转发 v1"| DAPI
  ST --> R
  MQ -->|"publish job status"| BR
  PMQ -->|"publish approval policy"| BR
  AD --> S3
  AD --> XFY
  AD --> DS
  AD --> VH
...
**注**：**`GET /v1/jobs/{id}`** 为只读查询；**`POST /v1/jobs/{id}/advance`** 触发 **后台** 单步推进（进程内队列或 BullMQ）。设备可 **轮询 GET** 观察状态直至 `preview_ready`。下图按 **阶段语义** 串联各档所调模块（非单次请求内顺序执行全部档）。

```mermaid
flowchart TB
  subgraph poll["POST v1 jobs jobId advance（后台 + Redis 锁）"]
    RQ[读取 Job]
...
**说明**：多副本时 **幂等创建** 与 **Job 正文** 存 **Redis**（或单机内存 + `JOBS_PERSISTENCE_PATH`）；流水线后台任务可选 **BullMQ**（`PIPELINE_QUEUE_BACKEND=bullmq`，见 **§7.1** / GitHub #13）。**冷导入 / 关机 SCAN 导出** 见 **§2.4.1** 实现注记；**遥测 NDJSON** 见 `DEVICE_TELEMETRY_LOG_PATH`。
...
与上表相关的 **仓库 `cloud/` 实现注记**（字段级仍以 OpenAPI 为准）：

| 环境变量 / 功能 | 说明 |
|----------------|------|
| `Idempotency-Key` | `POST /v1/jobs` 与 `POST .../print-ack` 按 **`device_id` + 键** 组合去重 |
| `POST .../chunks` 分片 | 支持 **`seq` + `audio_base64`** 分片，**`final:true`** 时解码拼接为整段音频（进程内缓冲，有总长度上限） |
| Job 推进流水线 | 关采音后由端上 **`POST /v1/jobs/{id}/advance`** 触发后台推进：ASR → 文本审核 → 生图与成图审核 → 预览定稿（未配置审核 HTTP 时对应步骤默认放行）；多副本时见 **`PIPELINE_QUEUE_BACKEND`** |
| `S3_AUDIO_BUCKET` | ASR 前将音频暂存对象存储并生成预签名 URL 供 **讯飞 IAT** 拉取 |
| 生图引擎 | 进程内 **通义万相（DashScope）**（`IMAGE_GEN_DRIVER`=`auto`/`tongyi`/`stub`） |
| 遥测审计 | `POST /v1/devices/telemetry` 与 MQTT 订阅在设置 **`DEVICE_TELEMETRY_LOG_PATH`** 时追加 **NDJSON** 审计行 |
| MQTT 订阅遥测 | **`MQTT_URL` + `MQTT_SUBSCRIBE_TELEMETRY=1`** 时 `device-api` 订阅 `devices/+/telemetry`（指标 `fancy_print_device_telemetry_mqtt_received_total`） |
| 链路追踪 | 网关与各 API 透传 **`traceparent` / `tracestate`** 与 **`X-Request-Id`** |
| mTLS | 见 [`cloud/docs/ingress-mtls.md`](../cloud/docs/ingress-mtls.md) |
| `DEVICE_REGISTRY_JSON_PATH` | 扩展设备凭证（可选 JSON 文件） |
| `JOBS_PERSISTENCE_PATH` | 无 `REDIS_URL` 时将 Job 与幂等键落盘为单实例 JSON |
| `REDIS_URL` | 多实例时 Job 与幂等键存 Redis |
| `JOB_REDIS_IMPORT_FILE=1` | 将 JSON 文件冷灌入 Redis |
| `JOB_FILE_EXPORT_ON_SHUTDOWN=1` | 进程退出前 **SCAN** 导出回文件 |
| 推进锁 | **`POST /v1/jobs/{id}/advance`** 在 Redis 下用 **`SET NX` + Lua 释放** 避免多副本双推一档（`JOB_ADVANCE_LOCK_*`） |
| `PIPELINE_QUEUE_BACKEND` | 默认进程内队列；设 **`bullmq`** 且已配置 **`REDIS_URL`** 时使用 **BullMQ** 分布式队列（多 `device-api` 副本共享），见 **GitHub #13** |
| `BULLMQ_PIPELINE_QUEUE_NAME` | BullMQ 队列名（默认 `fp:job-pipeline-advance`） |
| `PIPELINE_QUEUE_CONCURRENCY` | 进程内队列并发 **1～16**（默认 **2**）；BullMQ worker 并发 **1～128**（默认 **8**） |
| `DEVICE_API_CLUSTER_WORKERS` | **`>0`** 时 `npm run start:cluster` 以 Node **cluster** 多进程启动（单机多核）；与负载均衡 / 端口分配配合使用 |
| `MQTT_RECONNECT_MS` | MQTT 断线重连间隔（毫秒，默认 **1000**，范围约 **500～60000**） |
| `MQTT_SESSION_CLEAN` | `1`/`true`（默认）→ `clean` session；`0`/`false`/`no` → **`clean: false`**（断线后续传 QoS1 消息，与 broker 策略相关） |
...
| 场景 | 云端行为 | 端上配合 |
|------|----------|----------|
| ASR 上游失败 / 超时 | `job` 可进入 `failed` 或对应档重试策略（与 **`POST .../advance`** 后台推进实现一致）；**502/503** 见 OpenAPI | 弱网重试、话术引导 |
...
### 7.1 高并发与水平扩展（GitHub #13）

- **默认**：流水线后台任务在 **单进程内存队列**（`PipelineQueueService`）中执行，适合开发与小流量。  
- **水平扩展**：配置 **`REDIS_URL`**（Job 与幂等已共享）后，将 **`PIPELINE_QUEUE_BACKEND=bullmq`** 切换为 **BullMQ** 队列；各 `device-api` 副本内嵌 **Worker**，竞争消费同一队列，并可通过 **`PIPELINE_QUEUE_CONCURRENCY`** 提高单副本并行槽位（上限 128）。  
- **独立流水线 Worker（无 HTTP）**：同一镜像下设置 **`PIPELINE_WORKER_STANDALONE=1`**（且 **`bullmq` + `REDIS_URL`**）时进程不 `listen`，仅初始化 Nest 并消费 BullMQ，可与 API 副本分池扩缩；本地可用 **`npm run start:pipeline-worker -w device-api`**。  
- **容器与编排**：示例见仓库 **`cloud/deploy/`**（[`cloud/deploy/README.md`](../cloud/deploy/README.md)：`Dockerfile`、`docker-compose`、`kubectl` 示例）。  
- **多核**：可选 **`DEVICE_API_CLUSTER_WORKERS`** + **`npm run start:cluster`**（`device-api`）在同一主机 fork 多进程；生产环境通常以 **K8s 多副本 + BullMQ** 为主，cluster 为辅助手段。  
- **MQTT**：客户端启用 **`reconnectPeriod`**、**`resubscribe: true`** 及可配置的 **`clean` session**（见上表 `MQTT_*`）。
...
| [`2. 端侧软件与工程样机技术分析.md`](2. 端侧软件与工程样机技术分析.md) | 端侧 **完整**技术分析（OS、工程、安全、测试、**§10 BOM**、**§11 渲染**、**§13 Android 技术备案**） |
...
| [`../cloud/deploy/README.md`](../cloud/deploy/README.md) | **Docker / Compose / K8s** 示例与 **`PIPELINE_WORKER_STANDALONE`** 说明（与 **§7.1** 对齐） |
| [`6. 服务器运维手册.md`](6.%20服务器运维手册.md) | **一键部署脚本**、日常运维、排障、备份注意（Compose 全栈） |
